...
const ImagePreview: React.FC<ImagePreviewProps> = ({ selectedImageURL}) => {
    ...
    return (
        ...
        <Dialog.Trigger>
            <div className="photo-preview flex flex-auto gap-2 justify-center ">
                {/* Render the image four times with different aspect ratios */}
                {["9/16", "1/1", "4/3", "16/9"].map((aspectRatio, index) => (
                    
                    <button onClick={() => handleThumbnailClick(aspectRatio)} style={{aspectRatio:aspectRatio}} className={`relative result w-auto max-h-24 h-16 md:h-24 lg:h-24 bg-slate-400`} key={index}>
                        {selectedImageURL && (
                            <>
