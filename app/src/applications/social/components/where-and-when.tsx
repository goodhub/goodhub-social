...
function getNextPostingTime(currentDate: Date): Date {
    ...
    const nextPostingTime = new Date(nextWednesday.getFullYear(), nextWednesday.getMonth(), nextWednesday.getDate(), 14, 0, 0); // 2 PM
    ...
    return(
        <div className='md:text-lg'>
            <div className='flex mt-2 w-full justify-between md:justify-start items-center'>
                <div >
                    <span className='font-bold'>Posting to: </span><span>{`${getDescriptionsJoined()}`}</span>
                </div>  
            </div>
            <div className='flex justify-end'>
                <Button type="submit" variant="outline" onClick={() => toHelper(2)} className='rounded-full ml-2 py-0 px-2 h-7'>Change</Button>
            </div>
            <Separator className='my-4'/>
            <div className='flex mt-2 w-full justify-between md:justify-start items-center'>
                <div>
                    <span className='font-bold'>When: </span><span>{nextBestTime}</span>
                </div>  
            </div>
            <div className='flex justify-end'>
                <Button type="submit" variant="outline" onClick={() => toHelper(2)} className='rounded-full ml-2 py-0 px-2 h-7'>Change</Button>
            ...
            <div className='italic text-slate-600'>
                ...
                <p className='my-4'>You will be notified when it is posted, and you can access the post in your 'Social List' area.</p>
            </div>
        </div>
    )
